...
Every puretui module declares __all__; this suite pins the contract:
  - each __all__ name exists in its module (no stale exports),
  - __all__ entries are unique and never underscore-private,
  - every name puretui/__init__.py re-exports from a submodule is part of
    that submodule's declared public API,
  - puretui.__version__ matches the pyproject.toml version.
"""
...
import puretui
from puretui import (app, canvas, commands, interact, layout, region, styles,
...
class TestInitReexports(unittest.TestCase):
    def test_init_imports_are_in_source_all(self):
        ...
        member of puretui.mod.__all__ — the package facade cannot widen a
        module's public API."""
        tree = ast.parse(_read("puretui", "__init__.py"))
        checked = 0
        for node in ast.walk(tree):
            ...
            for alias in node.names:
                self.assertIn(alias.name, mod.__all__,
                              "puretui/__init__.py imports %r from puretui.%s but it "
